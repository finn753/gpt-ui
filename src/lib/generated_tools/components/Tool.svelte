...
<div class="">
    <h1 class="text-3xl pb-4 font-bold">{data.functionTree[0].name}</h1>
    <div class="flex justify-center space-x-16">
        <div class="">
            <form method="POST" on:submit|preventDefault={generateOutput}>
                <div class="grid xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 auto-cols-auto min-w-0 gap-4">
                    {#each data.functionTree[0].parameters as p}
                        {#if p.type === "string"}
                            {#if p.name.startsWith("color")}
                                <ColorParameter class="aspect-square max-w-[256px]" id={p.name} bind:value={input[p.name]}/>
                            {:else}
                                <TextParameter class="aspect-square max-w-[256px]" id={p.name} bind:value={input[p.name]}/>
                            {/if}
                        {/if}

                        {#if p.type === "string[]"}
                            <TextArrayParameter class="aspect-square max-w-[256px]" id={p.name} bind:value={input[p.name]}/>
                        {/if}

                        {#if p.type === "number"}
                            <NumberParameter class="aspect-square max-w-[256px]" id={p.name} bind:value={input[p.name]}/>
                        {/if}

                        {#if p.type === "number[]"}
                            <TextArrayParameter type="number" class="aspect-square max-w-[256px]" id={p.name} bind:value={input[p.name]}/>
                        {/if}

                        {#if p.type === "File[]"}
                            <FileParameter class="aspect-square max-w-[256px]" id={p.name} bind:value={input[p.name]}/>
                        {/if}
                    {/each}
                </div>
            </form>
        ...
        <div class="">
            <OutputRenderer data={data} outputPromise={output}/>
        </div>
    </div>
</div>