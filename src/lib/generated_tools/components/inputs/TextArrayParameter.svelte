...
<ToolCard class={cn("overflow-clip", className)}>
    ...
    <div class="p-2 space-y-1 overflow-y-auto">
        {#each textList as item, index}
            {#if type === 'text'}
                <input type="text" class={inputStyleClasses} id={index.toString()} bind:value={item} placeholder={id} use:init on:blur={handleBlur}/>
            {:else if type === 'number'}
                <input type="number" class={inputStyleClasses} id={index.toString()} bind:value={item} placeholder={id} use:init on:blur={handleBlur}/>
            {/if}
        {/each}
