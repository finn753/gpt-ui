...
<div class={cn("flex size-full", isRight ? "flex-row-reverse space-x-reverse" : "")}>
	<div
		class={cn(
		"size-full overflow-hidden rounded-r-2lg bg-level-two transition-all duration-500 shadow-lg",
	...
	>
		<div
			class={cn(
			"flex flex-row items-center justify-between bg-background p-4 shadow-lg"
		)}
		>
	...
	<Button variant="link" class="h-16 px-1 transition-all duration-500" on:click={() => {isOpen = !isOpen}}>
